...
public class RedisConfig {
    ...
    public RedisTemplate<Object, Object> redisTemplate(RedisConnectionFactory redisConnectionFactory) {
        ...
        //制定要序列化的域 filed get set 方法，修饰符 public private
        objectMapper.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
        //类必须是非final修饰
        objectMapper.enableDefaultTyping(ObjectMapper.DefaultTyping.NON_FINAL);
        jackson2JsonRedisSerializer.setObjectMapper(objectMapper);

        //将redis中的数据类型：进行了序列化与反序列化操作
        //redis， string hash 数据类型
        //set("user","userInfo.toString()") get("user") 序列化的对象：保持的二进制数据
        redisTemplate.setKeySerializer(new StringRedisSerializer()); //序列化与反序列化 String key
